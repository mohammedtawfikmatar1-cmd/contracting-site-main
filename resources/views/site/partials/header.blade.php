    ...
    <div class="container header-inner">
        ...
        <a href="{{ route('home') }}" class="logo" aria-label="موقع الشركة">
            <span class="logo-mark">
                <img src="{{ $settingsMedia('logo_main', asset('building.png'), asset('building.png')) }}"
                    alt="{{ $settingsValue(['company_name', 'site_name'], 'شعار الشركة', 'شعار الموقع') }}"
                    width="48" height="48" decoding="async">
            </span>
            <span
                class="logo-name">{{ $settingsValue(['company_name', 'site_name'], 'شركة مقاولات نموذجية', '') }}</span>
        </a>
        ...
        <div class="header-cluster">
            <nav class="nav nav--primary" aria-label="التنقل الرئيسي">
                <button class="menu-toggle" id="mobile-menu" type="button" aria-label="فتح القائمة"
                ...
                <ul id="primary-nav" class="nav-links" role="list">
                    ...
                    <li class="nav-mobile-cta">
                        <a href="{{ route('contact') }}#contact" class="nav-mobile-cta__link"> تواصل معنى </a>
                    </li>
                </ul>
        ...
        <div class="header-actions">
            <form action="{{ route('search') }}" method="GET" class="header-search-form" role="search">
                ...
                <input id="header-search-q" type="search" name="q" value="{{ request('q') }}"
                    placeholder="ابحث..." autocomplete="off" class="header-search-form__input">
