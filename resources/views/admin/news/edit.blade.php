    ...
    <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label>المحتوى</label>
                @if(!empty($enableMultilingual))
                    <textarea name="content[ar]" class="form-control js-editor mb-2" data-editor-context="news" rows="8">{{ old('content.ar', $news->getTranslation('content','ar')) }}</textarea>
                    <textarea name="content[en]" class="form-control js-editor" data-editor-context="news" rows="8">{{ old('content.en', $news->getTranslation('content','en')) }}</textarea>
                @else
                    <textarea name="content" class="form-control js-editor" data-editor-context="news" rows="10">{{ old('content', $news->getTranslation('content','ar') ?: $news->content) }}</textarea>
                @endif
            </div>
            <div class="form-group">
