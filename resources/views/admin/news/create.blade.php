            ...
            <form role="form" action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        @if(!empty($enableMultilingual))
                            <textarea name="content[ar]" class="form-control mb-2" rows="8" placeholder="محتوى الخبر (عربي)">{{ old('content.ar') }}</textarea>
                            <textarea name="content[en]" class="form-control" rows="8" placeholder="News content (EN)">{{ old('content.en') }}</textarea>
                        @else
